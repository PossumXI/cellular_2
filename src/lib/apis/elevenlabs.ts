...
export class ElevenLabsService {
  ...
  constructor() {
    ...
    console.log('🎵 ElevenLabs service initialized with official SDK');
    this.validateApiKey();
  }

  private isValidApiKey(key: string): boolean {
    return key && key.startsWith('sk_') && key.length > 20 && key !== 'your_elevenlabs_api_key_here';
  }

  private async validateApiKey(): Promise<boolean> {
    if (this.isKeyValid !== null) {
      return this.isKeyValid;
    }

    if (!this.isValidApiKey(config.elevenlabs.apiKey)) {
      console.warn('🎵 ElevenLabs API key is not configured properly. Please set VITE_ELEVENLABS_API_KEY in your .env file.');
  ...
  async getAvailableVoices() {
    try {
      ...
      if (!isValid) {
        console.warn('ElevenLabs API key is not valid - returning empty voice list');
        return [];
      }
  ...
  async synthesizeVoice(
    ...
    try {
      const isValid = await this.validateApiKey();
      if (!isValid) {
        console.warn('🎵 Voice synthesis skipped - API key is not valid');
        return null;
      }

      // Use the correct method from the official SDK
      const audioStream = await this.client.textToSpeech.convert(voiceProfile.voiceId, {
        text,
  ...
  async playLocationVoice(text: string, voiceProfile: VoiceProfile, location?: LocationCell) {
    try {
      ...
      const isValid = await this.validateApiKey();
      if (!isValid) {
        console.log('🎵 Voice synthesis disabled - API key not configured or invalid');
        return { 
          success: false, 
          error: 'Voice synthesis requires a valid ElevenLabs API key. Please configure VITE_ELEVENLABS_API_KEY in your .env file.',
  ...
  // Test voice synthesis with a sample
  async testVoiceSynthesis(): Promise<boolean> {
    try {
      ...
      const result = await this.synthesizeVoice('Testing voice synthesis', testVoice);
      return result !== null;
    } catch (error) {
