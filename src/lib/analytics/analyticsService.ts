...
export class AnalyticsService {
  ...
  private cachedData: Record<string, {data: any, timestamp: number}> = {};
  ...
  async initialize(): Promise<void> {
    ...
    try {
      // Start data collection with reduced frequency
  ...
  async getSocialAnalytics(): Promise<any> {
    try {
      // Get social engagement analytics - select only scalar columns to avoid array dimensionality issues
      const { data: socialEngagement, error: socialError } = await supabase
        .from('live_social_engagement')
        .select('location_name,total_posts_last_hour,total_likes_last_hour,total_retweets_last_hour,avg_sentiment_last_hour,avg_engagement_rate,data_points')
      ...
      // Aggregate hashtags with type safety
      const allHashtags: string[] = [];
      
      // Safely process hashtags to avoid dimensionality issues
      if (hashtagData && Array.isArray(hashtagData)) {
        hashtagData.forEach(item => {
          if (item && item.trending_hashtags && Array.isArray(item.trending_hashtags)) {
            item.trending_hashtags.forEach(tag => {
      ...
      const overallSentiment = sentimentData && sentimentData.length > 0
